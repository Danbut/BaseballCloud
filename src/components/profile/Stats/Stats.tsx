...
const Stats: VFC<{ id: string; profile: Profile }> = ({ id, profile }) => {
  ...
  subTabs.set('1', <Charts id={id} />);
  subTabs.set('2', <Log id={id} />);
    ...
    <Card bg="white" m="16px" p="16px" borderRadius="8px">
      <StyledTabs>
        ...
        <StyledTabsPanel
          height="100%"
          minHeight="420px"
          index={parseInt(activeTabIndex) + 1}
        >
          <TabPanel>{subTabs.get(activeSubTabIndex)}</TabPanel>
          <TabPanel>
            <SessionReports id={id} />
          </TabPanel>
          <TabPanel>
